...
void actualizarViajesEstado(Estr_Viaje *viaje, int numViajes, Estr_Reservas *reservas, int numReservas, Estr_Pasos *pasos, int numPasos)
{
    ...
    else
    {
        for(i=0; i<numViajes; i++) //Nos desplazamos por todal la estructura "Viajse"
        {
            ...
            struct tm tiempo_introducido={0}; //Definimos estructura para poner anos desde 2023, meses y dias.
...
int dias_meses(int mes, int ano)
{
    ...
    else if (mes==2)
    {
        ...
        {
            dias=29; //febrero tiene 29 dias en anos bisiestos
        }
        else
        {
            dias=28; //febrero tiene 28 dias en anos no bisiestos
        }
    }
