...
void verificar_reserva(Estr_Usuario *, Estr_Vehiculo *, int, Estr_Viaje *, int, Estr_Reservas *, int, int *, int, int *, int);
            ...
            do
            {
                ...
                switch(opc)
                {
                    case 1:
                        ...
                        menuAdminViajes(usuario, numUsuarios, vehiculo, numVehiculos, viaje, numViajes, pasos, numPasos, reservas, numReservas, localidad, numLocalidades, ruta, numRutas, numRutas2, 0);
                        break;
                    case 2:
                        breakp=1;
                        menuAdminViajes(usuario, numUsuarios, vehiculo, numVehiculos, viaje, numViajes, pasos, numPasos, reservas, numReservas, localidad, numLocalidades, ruta, numRutas, numRutas2, 0);
...
void altaReserva(Estr_Usuario *usuario, int numUsuarios, Estr_Vehiculo *vehiculo, int numVehiculos, Estr_Viaje *viaje, int numViajes, Estr_Pasos *pasos, int numPasos, Estr_Reservas *reservas, int numReservas, int num_user)
{
    ...
    do //se repite el bucle hasta que los datos ingresados esten correctos
    {
        ...
        else //caso contrario exiten viajes para la fecha
        {
            ...
            verificar_reserva(usuario, vehiculo, numVehiculos, viaje, numViajes, reservas, numReservas, vec, seleccion-1, &conf1, num_user);
...
void mostrar_poblaciones(Estr_Viaje *viaje, int numViajes, Estr_Pasos *pasos, int numPasos, int *vec, int reserva, int *conf)
{
    ...
    printf("Viaje seleccionado para el %s\n", viaje[vec[reserva]].f_inic);
    color(0, 3);
    printf("ID del viaje: ");
    color(0, 15);
...
//Cabecera: void verificar_reserva(Estr_Usuario *, Estr_Vehiculo *, int, Estr_Viaje *, int, Estr_Reservas *, int, int *, int, int *, int);
...
void verificar_reserva(Estr_Usuario *usuario, Estr_Vehiculo *vehiculo, int numVehiculos, Estr_Viaje *viaje, int numViajes, Estr_Reservas *reservas, int numReservas, int *vec, int reserva, int *n, int num_user)
{
    int i, j;
    ...
    if((*n)!=1)
    {
        ...
        {
            if(strcmp(viaje[reserva].id_viaje, reservas[j].id_viaje)==0)
            {
